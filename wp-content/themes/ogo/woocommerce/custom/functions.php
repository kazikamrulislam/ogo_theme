...
class WC_Functions {
	...
	public function __construct() {
		...
		add_action( 'loop_shop_per_page',                  array( $this, 'ogo_loop_shop_per_page' ), 20 );
		add_action( 'woocommerce_after_shop_loop',         array( $this, 'ogo_products_paginations' ), 10 );
		// Columns
		add_filter( 'loop_shop_columns',                    array( $this, 'loop_shop_columns' ) );
		add_filter( 'woocommerce_get_image_size_thumbnail', array( $this, 'ogo_product_imgae_size' ) );
		add_filter( 'woocommerce_get_image_size_single', array( $this, 'ogo_single_product_imgae_size' ) );
		add_filter( 'woocommerce_get_image_size_gallery_thumbnail', array( $this, 'ogo_product_thumbnail_imgae_size' ) );
		...
		if ( function_exists( 'YITH_WCWL_Frontend' ) && class_exists( 'YITH_WCWL_Ajax_Handler' )  ) {
			...
			add_action( 'wp_ajax_ogo_add_to_wishlist',                array( $this, 'add_to_wishlist' ) );
			add_action( 'wp_ajax_nopriv_ogo_add_to_wishlist',         array( $this, 'add_to_wishlist' ) );
		}
		...
		/* ====== Checkout Page ====== */
		add_filter( 'woocommerce_checkout_fields', array( $this, 'ogo_checkout_fields' ) );

		/* ====== Mini Cart ====== */
		add_action( 'ogo_woo_cart_icon', array( $this, 'ogo_wc_cart_count' ) );
		add_action( 'woocommerce_add_to_cart_fragments', array( $this, 'ogo_header_add_to_cart_fragment' ) );
		add_action( 'wp_ajax_ogo_product_remove', array( $this, 'ogo_product_remove' ) );
        add_action( 'wp_ajax_nopriv_ogo_product_remove', array( $this, 'ogo_product_remove' ) );
	}
	...
	public function loop_shop_columns(){
		$cols = OgoTheme::$options['products_cols_width'];
		return $cols;
	}

	public function ogo_product_imgae_size( $size ) {
		return array(
			'width' => 370,
			'height' => 450,
			'crop' => 1,
	...
	public function ogo_product_thumbnail_imgae_size( $size ) {
		return array(
			'width' => 155,
			'height' => 157,
			'crop' => 0,
	...
	public function ogo_single_product_imgae_size( $size ) {
		return array(
			'width' => 450,
			'height' => 515,
			'crop' => 1,
	...
	public function ogo_products_paginations(){
		OgoTheme_Helper::pagination();
	}
	...
	public function single_socials_share(){
		if( OgoTheme::$options['wc_product_social_icon'] ){
			echo '<div class="product-single-social-shares-btns">';
			self::ogo_product_social_share();
			echo '</div>';
		}
	}	
    ...
    public function ogo_loop_shop_per_page( $products ) {
        // Return the number of products you wanna show per page.
        $shop_posts_per_page = OgoTheme::$options['products_per_page'];
	...
	public static function ogo_product_social_share(){
		...
		$sharers = apply_filters( 'rttheme_social_sharing_icons', $defaults );

		?>
		<div class="post-share-btn">
			<h5 class="item-label"><?php esc_html_e( 'Share:', 'ogo' );?></h5>
	...
	public static function print_quickview_icon( $icon = true, $text = false ){
		...
		if ( is_shop() && !OgoTheme::$options['wc_shop_quickview_icon'] ) {
			return false;
		}

		if ( is_product() && !OgoTheme::$options['wc_product_quickview_icon'] ) {
			return false;
		}
		...
		if ( $text ) {
			$html .= '<span>' . esc_html__( 'QuickView', 'ogo' ) . '</span>';
		}

		?>
		<a href="#" class="yith-wcqv-button" data-product_id="<?php echo esc_attr( $product->get_id() );?>" title="<?php esc_attr_e( 'QuickView', 'ogo' ); ?>"><?php echo wp_kses_post( $html ); ?></a>
		<?php
	}
	...
	public static function print_compare_icon( $icon = true, $text = true ){
		...
		if ( is_shop() && !OgoTheme::$options['wc_shop_compare_icon'] ) {
			return false;
		}

		if ( is_product() && !OgoTheme::$options['wc_product_compare_icon'] ) {
			return false;
		}
		...
		?>
		<a href="<?php echo esc_url( $url );?>" class="compare" data-product_id="<?php echo esc_attr( $id );?>" title="<?php esc_attr_e( 'Add To Compare', 'ogo' ); ?>"><?php echo wp_kses_post( $html ); ?></a>
		<?php
	}
	...
	public static function print_add_to_wishlist_icon( $icon = true, $text = false ){
		...
		if ( is_shop() && !OgoTheme::$options['wc_shop_wishlist_icon'] ) {
			return false;
		}

		if ( is_product() && !OgoTheme::$options['wc_product_wishlist_icon'] ) {
			return false;
		}
	...
	public static function get_stock_status() {
		global $product;
		return $product->is_in_stock() ? esc_html__( 'In Stock', 'ogo' ) : esc_html__( 'Out of Stock', 'ogo' );
	}

	// WooCommerce Checkout Fields Hook
    public function ogo_checkout_fields( $fields ) {
    ...
    public static function OgoWooMiniCart(){
        ob_start();
        if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
            do_action( 'ogo_woo_cart_icon' );
        }
     ...
     */
    public function ogo_cart_items(){
        $empty_cart = '<li class="cart-item d-flex align-items-center"><h5 class="text-center no-cart-items">'. apply_filters( 'ogo_woo_mini_cart_empty', esc_html__('Your cart is empty', 'ogo') ) .'</h5></li>';
        ...
        $remove_loader = apply_filters('woo_mini_cart_loader', OgoTheme_Helper::get_img( 'spinner2.gif' ));
                ...
                <li class="cart-item d-flex align-items-center">
	                ...
	                <div class="cart-single-product">
						<div class="media">
										...
										<?php
				                            echo apply_filters(
				                                ...
				                                sprintf(
				                                    ...
				                                    esc_attr__( 'Remove this item', 'ogo' ),
				                                    esc_attr( $product_id ),
							...
							<span class="remove-item-overlay text-center"><img src="<?php echo esc_url($remove_loader); ?>" alt="<?php esc_attr_e('Loader..', 'ogo') ?>" /></span>
						</div>
					</div>
            ...
            <li class="cart-total">
                <div class="total-price">
                    <span class="f-left"><?php _e( 'Total:', 'ogo' ); ?></span>
                    <span class="f-right"><?php echo WC()->cart->get_cart_subtotal(); ?></span>
                </div>
            </li>
            <li class="cart-btn">
                <div class="checkout-link">
                    <a class="button wc-forward" href="<?php echo wc_get_cart_url(); ?>"><?php _e( 'View Cart', 'ogo' ); ?></a>
                    <a class="button wc-forward checkout" href="<?php echo wc_get_checkout_url(); ?>"><?php _e( 'Checkout', 'ogo' ); ?></a>
                </div>
            </li>
            <?php endif; ?>
        <?php 
    ...
    public function ogo_wc_cart_count() {
        ...
        if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
            ...
            <div class="cart-list-trigger">
	            <a class="cart-contents cart-trigger-icon" href="<?php echo esc_url( $cart_link ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'ogo' ); ?>"><i class="fas fa-shopping-cart"></i> <?php if ( $count > 0 ) echo '(' . $count . '2' . ')'; ?></a>
                <div class="cart-wrapper">
                    <ul class="minicart">
		            	<?php echo wp_kses_stripslashes( $this->ogo_cart_items() ); ?>
		            </ul>
                </div>
            </div>
            <?php
     ...
     */
    public function ogo_header_add_to_cart_fragment( $fragments ) {
        ob_start();
        ...
        ?>
            <div class="header-action-item cart-area mini-cart-items header-shop-cart">
            	<div class="cart-list-trigger">
	                <a class="cart-contents cart-trigger-icon" href="<?php echo esc_url( $cart_link ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'ogo' ); ?>"><i class="fas fa-shopping-cart"></i> <?php if ( $count > 0 ) echo '<span>' . $count . '</span>'; ?></a>
	                <div class="cart-wrapper">
		                <ul class="minicart">
		                <?php echo wp_kses_stripslashes( $this->ogo_cart_items() ); ?>
		                </ul>
    ...
    public function ogo_wc_cart_ajax() {
        $output = '';
        if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
        	if(!is_null(WC()->cart)) {
			    $count = WC()->cart->cart_contents_count;
            ...
            ob_start();
            ?>
            <a class="cart-contents" href="<?php echo esc_url( $cart_link ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'ogo' ); ?>"><i class="fas fa-shopping-cart"></i> <?php if ( $count > 0 ) echo '(' . $count . ')'; ?></a>
            <ul class="minicart">
            <?php
                echo wp_kses_stripslashes( $this->ogo_cart_items() );
            ?>
            </ul>
            <?php
            $output = ob_get_clean();
    ...
    public function ogo_product_remove() {
        ...
        $return["mini_cart"] = $this->ogo_wc_cart_ajax();
