...
if( ! function_exists( 'ogo_related_team' )){
	
	function ogo_related_team(){
		$thumb_size = 'ogo-size6';
		$post_id = get_the_id();	
		$number_of_avail_post = '';
		$current_post = array( $post_id );
		$title_length = OgoTheme::$options['related_team_title_limit'] ? OgoTheme::$options['related_team_title_limit'] : '';
		$related_post_number = OgoTheme::$options['related_team_number'];
		...
		# Get related posts by categories ----------
			...
			$terms = get_the_terms( $post_id, 'ogo_team_category' );
		...
		if ( OgoTheme::$options['related_team_auto_play'] == 1 ) {
			$auto_play = true;
		} else {
			$auto_play = false;
		}

		$speed = intval(OgoTheme::$options['related_team_auto_play_speed']);
		$delay = intval(OgoTheme::$options['related_team_auto_play_delay']);
		...
		<div class="team-default team-multi-layout-1 team-grid-style1">
			<div class="amt-swiper-slider-team related-post" data-xld = '<?php echo esc_attr( $swiper_data ); ?>'>
				<div class="section-title">
					<h3 class="related-title"><?php echo wp_kses( OgoTheme::$options['team_related_title'] , 'alltext_allow' ); ?>
								...
								<div class="team-thums">
									<a href="<?php the_permalink();?>">
										...
										if ( has_post_thumbnail() ){
											...
											echo '<img class="wp-post-image" src="' . OgoTheme_Helper::get_asset_file( 'element/noimage_400X400.jpg' ) . '" alt="'.get_the_title().'">';
										}
										?>
									</a>
								</div>
