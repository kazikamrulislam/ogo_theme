...
if ( !class_exists( 'OgoTheme_Helper' ) ) {
	
	class OgoTheme_Helper {
		...
		public static function pagination() {
			...
			include OGO_VIEW_DIR . 'pagination.php';
		}	

		public static function comments_callback( $comment, $args, $depth ){
			include OGO_VIEW_DIR . 'comments-callback.php';
		}
		...
		public static function fonts_url(){
			...
			// Body Font
			$body_font  = json_decode( OgoTheme::$options['typo_body'], true );
			...
			// Menu Font
			$menu_font  = json_decode( OgoTheme::$options['typo_menu'], true );
			...
			$h_font  = json_decode( OgoTheme::$options['typo_heading'], true );
			...
			$h1_font  = json_decode( OgoTheme::$options['typo_h1'], true );
			$h2_font  = json_decode( OgoTheme::$options['typo_h2'], true );
			$h3_font  = json_decode( OgoTheme::$options['typo_h3'], true );
			$h4_font  = json_decode( OgoTheme::$options['typo_h4'], true );
			$h5_font  = json_decode( OgoTheme::$options['typo_h5'], true );
			$h6_font  = json_decode( OgoTheme::$options['typo_h6'], true );

			if ( 'off' !== _x( 'on', 'Google font: on or off', 'ogo' ) ) {
				...
				// Body Font
				if ( 'off' !== $bodyFont )
					$font_families[] = $bodyFont.':300,400,600,'.$bodyFW;
					$check_families[] = $bodyFont;

				// Menu Font
				if ( 'off' !== $menuFont )
					if ( !in_array( $menuFont, $check_families ) ) {
						$font_families[] = $menuFont.':300,400,700,'.$menuFontW;
				...
				if ( 'off' !== $hFont )
					if (!in_array( $hFont, $check_families )) {
						$font_families[] = $hFont.':300,400,700,'.$hFontW;
		...
		public static function socials(){
			$ogo_socials = array(
				'social_facebook' => array(
					'icon' => 'fa-facebook-f',
					'url'  => OgoTheme::$options['social_facebook'],
				),
				'social_twitter' => array(
					'icon' => 'fa-twitter',
					'url'  => OgoTheme::$options['social_twitter'],
				),
				'social_behance' => array(
					'icon' => 'fa-behance',
					'url'  => OgoTheme::$options['social_behance'],
				),
				'social_dribbble' => array(
					'icon' => 'fa-dribbble',
					'url'  => OgoTheme::$options['social_dribbble'],
				),
				'social_linkedin' => array(
					'icon' => 'fa-linkedin-in',
					'url'  => OgoTheme::$options['social_linkedin'],
				),
				'social_youtube' => array(
					'icon' => 'fa-youtube',
					'url'  => OgoTheme::$options['social_youtube'],
				),
				'social_pinterest' => array(
					'icon' => 'fa-pinterest-p',
					'url'  => OgoTheme::$options['social_pinterest'],
				),
				'social_instagram' => array(
					'icon' => 'fa-instagram',
					'url'  => OgoTheme::$options['social_instagram'],
				),
				'social_skype' => array(
					'icon' => 'fa-skype',
					'url'  => OgoTheme::$options['social_skype'],
				),
				'social_rss' => array(
					'icon' => 'fas fa-rss',
					'url'  => OgoTheme::$options['social_rss'],
				),
				'social_snapchat' => array(
					'icon' => 'fa-snapchat-ghost',
					'url'  => OgoTheme::$options['social_snapchat'],
				),
			);
			return array_filter( $ogo_socials, array( 'OgoTheme_Helper' , 'filter_social' ) );
		}
		public static function team_socials(){
			$team_socials = array(
				'facebook' => array(
					'label' => esc_html__( 'Facebook', 'ogo' ),
					'type'  => 'text',
					'icon'  => 'fa-facebook-f',
				),
				'twitter' => array(
					'label' => esc_html__( 'Twitter', 'ogo' ),
					'type'  => 'text',
					'icon'  => 'fa-twitter',
				),
				'linkedin' => array(
					'label' => esc_html__( 'Linkedin', 'ogo' ),
					'type'  => 'text',
					'icon'  => 'fa-linkedin-in',
				),
				'behance' => array(
					'label' => esc_html__( 'Behance', 'ogo' ),
					'type'  => 'text',
					'icon'  => 'fa-behance',
				),
				'dribbble' => array(
					'label' => esc_html__( 'Dribbble', 'ogo' ),
					'type'  => 'text',
					'icon'  => 'fa-dribbble',
				),
				'skype' => array(
					'label' => esc_html__( 'Skype', 'ogo' ),
					'type'  => 'text',
					'icon'  => 'fa-skype',
				),
				'youtube' => array(
					'label' => esc_html__( 'Youtube', 'ogo' ),
					'type'  => 'text',
					'icon'  => 'fa-youtube',
				),
				'pinterest' => array(
					'label' => esc_html__( 'Pinterest', 'ogo' ),
					'type'  => 'text',
					'icon'  => 'fa-pinterest-p',
				),
				'instagram' => array(
					'label' => esc_html__( 'Instagram', 'ogo' ),
					'type'  => 'text',
					'icon'  => 'fa-instagram',
				),
				'github' => array(
					'label' => esc_html__( 'Github', 'ogo' ),
					'type'  => 'text',
					'icon'  => 'fa-github',
				),
				'stackoverflow' => array(
					'label' => esc_html__( 'Stackoverflow', 'ogo' ),
					'type'  => 'text',
					'icon'  => 'fa-stack-overflow',
				),
			);
			return apply_filters( 'OgoTheme_Helper', $team_socials );
		}

		public static function nav_menu_args(){
			$ogo_pagemenu = false;			
			if ( ( is_single() || is_page() ) ) {
				$layout_settings2 = get_post_meta( get_the_id(), 'ogo_layout_settings', true );
				if ( !empty( $layout_settings2 ) ) {
					$ogo_menuid = $layout_settings2['ogo_page_menu'];
				} else {
					$ogo_menuid = '';
				}
				if ( !empty( $ogo_menuid ) && $ogo_menuid != 'default' ) {
					$ogo_pagemenu = $ogo_menuid;
				}
			}
			if ( $ogo_pagemenu ) {
				$nav_menu_args = array( 'menu' => $ogo_pagemenu,'container' => 'nav' );
			}
			else {
		...
		public static function has_footer(){
			...
			if ( OgoTheme::$footer_style == 2  ) {
				$footer_column = OgoTheme::$options['footer_column_2'];
				...
				return false;
			} else if ( OgoTheme::$footer_style == 3 ) {
				$footer_column = OgoTheme::$options['footer_column_3'];
				for ( $i = 1; $i <= $footer_column; $i++ ) {
					if ( is_active_sidebar( 'footer-'. $i ) ) {
						return true;
					}
				...
				return false;				
			} else if ( OgoTheme::$footer_style == 4 ) {
				$footer_column = OgoTheme::$options['footer_column_4'];
				for ( $i = 1; $i <= $footer_column; $i++ ) {
					if ( is_active_sidebar( 'footer-'. $i ) ) {
						return true;
					}
				...
				return false;
			} else if ( OgoTheme::$footer_style == 5 ) {
				$footer_column = OgoTheme::$options['footer_column_5'];
				for ( $i = 1; $i <= $footer_column; $i++ ) {
					if ( is_active_sidebar( 'footer-'. $i ) ) {
						return true;
					}
		...
		public static function get_element_img( $img ){
			$img = get_stylesheet_directory_uri() . '/assets/element/' . $img;
			return $img;
		}
		...
		public static function get_font_css( $filename ){
			$path = '/assets/fonts/flaticon-ogo/' . $filename . '.css';
		...
		public static function has_active_widget(){
			...
			if ( ( class_exists( 'WooCommerce' ) && is_shop() ) || ( class_exists( 'WooCommerce' ) && is_product_category() ) || ( class_exists( 'WooCommerce' ) && is_product_tag() ) || ( class_exists( 'WooCommerce' ) && is_product() ) ) {	
				...
				if ( is_active_sidebar( 'shop-sidebar-1' ) ) {
					$ogo_layout_class = "col-xl-8 col-lg-8 {$sidebar_order}";
				} else {
			...
			if ( OgoTheme::$sidebar ) {
				if ( is_active_sidebar( OgoTheme::$sidebar ) ) {
					$ogo_layout_class = "col-xl-8 col-lg-8 {$sidebar_order}";
				} else {
					$ogo_layout_class = 'col-12';
				}
		...
		public static function custom_sidebar_fields(){
			...
			$sidebar_fields['sidebar'] = esc_html__( 'Sidebar', 'ogo' );

			$sidebars = get_option( 'ogo_custom_sidebars', array() );
			if ( $sidebars ) {
		...
		public static function dynamic_internal_style(){
			ob_start();
			include OGO_INC_DIR . 'variable-style.php';
			include OGO_INC_DIR . 'variable-style-elementor.php';
			$dynamic_css  = ob_get_clean();
			$dynamic_css  = self::minified_css( $dynamic_css );
			wp_register_style( 'ogo-dynamic', false );
			wp_enqueue_style( 'ogo-dynamic' );
			wp_add_inline_style( 'ogo-dynamic', $dynamic_css );
		}
	}
}