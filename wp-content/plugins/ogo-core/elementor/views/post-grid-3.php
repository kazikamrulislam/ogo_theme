...
$ogo_has_entry_meta  = ( $data['post_author'] == 'yes' || $data['post_cats'] == 'yes' || $data['post_date'] == 'yes' || $data['post_comment'] == 'yes' || $data['post_length'] == 'yes' && function_exists( 'ogo_reading_time' ) || $data['post_view'] == 'yes' && function_exists( 'ogo_views' ) ) ? true : false;
...
<div class="amt-post-grid-default amt-post-grid-<?php echo esc_attr( $data['style'] );?>">
	...
	<div class="text-center blog-loadmore">
		...
		<?php if ( $data['loadmore_display'] == 'yes' ) { ?>
	      	<a href="#" id="blog_loadMore_style3" class="loadMore" data-loadmore='<?php echo esc_attr( $encoded_data ) ;?>'><?php echo wp_kses_post( $data['button_text'] ); ?></a>
	      <?php } ?>
    </div>
