...
/* Post reading time */
if( !function_exists( 'ogo_reading_time' )){

	function ogo_reading_time(){
		...
		if( $reading_time < 1){
			$result = esc_html__( 'Less than a minute', 'ogo-core' );
		}
		elseif( $reading_time > 60 ){
			$result = sprintf( esc_html__( '%s hours read' ), floor( $reading_time / 60 ) );
		}
		else if ( $reading_time == 1 ){
			$result = esc_html__( '1 minute read', 'ogo-core' );
		} else {
			$result = sprintf( esc_html__( '%s minutes read', 'ogo-core' ), $reading_time );
		}
