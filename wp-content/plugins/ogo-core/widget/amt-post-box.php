...
* By : Radius Theme
*/
Class OgoTheme_Post_Box extends WP_Widget {
	public function __construct() {
		$widget_ops = array(
			'classname' => 'amt-post-box',
			'description' => esc_html__( 'Post Display Widget' , 'ogo-core' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct( 'amt-post-box', esc_html__( 'Ogo : Posts display Box' , 'ogo-core' ), $widget_ops );
	}
	public function widget( $args, $instance ) {
		...
		$title = ( !empty( $instance['title'] ) ) ? $instance['title'] : esc_html__( '' , 'ogo-core' );
		...
		<?php while ( $result_query->have_posts() ) : $result_query->the_post(); ?>
			...
			<div class="amt-news-box-widget <?php echo esc_attr( $select_style ); ?>">
				<div class="item-list">
					<?php if ( $show_post_image == 'yes' ) { ?>
						<div class="post-box-img">
							<a class="post-img" href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php
								if ( has_post_thumbnail() ){
									the_post_thumbnail( 'ogo-size5', ['class' => 'media-object'] );
			...
			<div class="col-6 <?php echo esc_attr( $select_style ); ?>">
				<div class="topic-box">				
					<?php if ( $show_post_image == 'yes' ) { ?>
						<a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>" class="amt-wid-post-img img-opacity-hover">
							<?php
								if ( has_post_thumbnail() ){
									the_post_thumbnail( 'ogo-size3', ['class' => 'media-object'] );
								} elseif ( OgoTheme::$options['display_no_preview_image'] == '1' ) {
	...
	public function form( $instance ) {
		...
		?>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_cat' )); ?>"><?php esc_html_e( 'Display post category?', 'ogo-core' ); ?></label></p>
			
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'select_style' )); ?>"><?php esc_html_e( 'Select Box Style : ', 'ogo-core' ); ?></label>
				<select name="<?php echo esc_attr( $this->get_field_name( 'select_style' )); ?>">
					<option <?php if ( $select_style == 'box-style-1' ) {  echo 'selected'; } ?> value="box-style-1"><?php esc_html_e( 'Style 1' , 'ogo-core' ); ?></option>
					<option <?php if ( $select_style == 'box-style-2' ) {  echo 'selected'; } ?> value="box-style-2"><?php esc_html_e( 'Style 2' , 'ogo-core' ); ?></option>
					<option <?php if ( $select_style == 'box-style-3' ) {  echo 'selected'; } ?> value="box-style-3"><?php esc_html_e( 'Style 3' , 'ogo-core' ); ?></option>
					<option <?php if ( $select_style == 'box-style-4' ) {  echo 'selected'; } ?> value="box-style-4"><?php esc_html_e( 'Style 4' , 'ogo-core' ); ?></option>
				</select>
			</p>
			<p>
