...
if( !class_exists( 'AMT_Postmeta_Fields' ) ){
	...
	class AMT_Postmeta_Fields {
		...
		public function display_fields( $fields, $post_id ){
			echo '<table class="amt-postmeta-container">';
		...
		private function display_repeater_field( $key, $field, $post_id ){
			...
			echo '<tr><td colspan="2" class="amt-postmeta-repeater-wrap" data-num="'.$count.'" data-fieldname="'. esc_attr( $key ) .'">';

			// First Hidden Item
			echo '<table class="amt-postmeta-repeater repeater-init">';
			foreach ( $field['value'] as $key2 => $field2 ) {
			...
			if ( !empty( $meta ) ){
				foreach ( $meta as $item => $itemvalue ) {
					echo '<table class="amt-postmeta-repeater">';
			...
			$buttontext = empty( $field['button'] ) ? esc_html__( 'Add More', 'amt-framework' ) : $field['button'];
			echo '<div class="amt-postmeta-repeater-addmore"><button>'. $buttontext .'</button></div></td></tr>';
		}

		private function display_single_field( $key, $field, $post_id, $default = false ){
			$desc = '';
			if ( !empty( $field['desc'] ) ){
				$desc = '<div class="amt-postmeta-desc">' . wp_kses_post( $field['desc'] ) . '</div>';
			}

			$container_attr = '';
			if ( !empty( $field['required'] ) ) {
				$container_attr .= ' class="amt-postmeta-dependent"';
		...
		public function multi_select( $key, $field, $default, $class ){
			...
			echo '<select class="amt-multi-select" data-placeholder=" ' . esc_attr__( 'Click here to select options', 'amt-framework' ) . '" multiple="multiple"'.
			' name="' . esc_attr( $key ) . '[]"'.
			' id="' . esc_attr( $key ) . '">';
			foreach ( $field['options'] as $key => $value ) {
				echo '<option',
		...
		public function multi_checkbox( $key, $field, $default, $class ){
			...
			foreach ( $field['options'] as $value => $title ) {
				...
				echo '<span class="amt-postmeta-radio"><input type="checkbox"'. $class .
				' name="' . esc_attr( $key ) . '[]"'.
				' id="' . esc_attr( $id ) . '"'.
		...
		public function radio( $key, $field, $default, $class ){
			foreach ( $field['options'] as $value => $title ) {
				...
				echo '<span class="amt-postmeta-radio"><input type="radio"'. $class .
				' name="' . esc_attr( $key ) . '"'.
				' id="' . esc_attr( $id ) . '"'.
				' value="' . esc_attr( $value ) . '"',
		...
		public function image( $key, $field, $default, $class ){
			...
			<div class="amt_metabox_image">
				...
				<input class="rt_upload_image upload_button_'. esc_attr( $key ) .' button-primary" type="button" value="' . esc_attr__( 'Choose Image', 'amt-framework' ). '" />
				<div class="rt_remove_image_wrap" style="'. esc_attr( $disstyle ) .'"><a href="#" class="rt_remove_image button" >' . esc_html__( 'Remove Image', 'amt-framework' ). '</a></div>
			</div>
			';
		}
		...
		public function gallery( $key, $field, $default, $class ){
			...
			<div class="amt_metabox_gallery">
				...
				<div class="amt_metabox_gallery_buttons">
					<input class="rt_upload_gallery upload_button_'. esc_attr( $key ) .' button-primary" type="button" value="' . esc_attr__( 'Add/Edit Gallery Images', 'amt-framework' ). '" />
					<a href="#" style="'. esc_attr( $disstyle ) .'" class="rt_remove_gallery button" >' . esc_html__( 'Remove All Images', 'amt-framework' ). '</a>
				</div>
			</div>
			';
		}
		...
		public function file( $key, $field, $default, $class ){
			...
			<div class="amt_metabox_file">
				...
				<input class="rt_upload_file upload_button_'. esc_attr( $key ) .' button-primary" type="button" value="' . esc_attr__( 'Choose File', 'amt-framework' ). '" />
				<div class="rt_remove_file_wrap" style="'. esc_attr( $disstyle ) .'"><a href="#" class="rt_remove_file button" >' . esc_html__( 'Remove File', 'amt-framework' ). '</a></div>
		...
		public function color_picker( $key, $field, $default, $class ){
			echo '<input type="text" class="amt-metabox-picker amt-metabox-colorpicker" name="' . esc_attr( $key ) . '" id="' . esc_attr( $key ) . '" value="' . esc_attr( $default ) . '" />';
		}

		public function date_picker( $key, $field, $default, $class ){
			$format = isset( $field['format'] ) ? $field['format'] : 'MM dd, yy';
			echo '<input type="text" data-format="' . $format . '" class="amt-metabox-picker amt-metabox-datepicker" name="' . esc_attr( $key ) . '" id="' . esc_attr( $key ) . '" value="' . esc_attr( $default ) . '" />';
		}

		public function time_picker( $key, $field, $default, $class ){
			echo '<input type="text" class="amt-metabox-picker amt-metabox-timepicker" name="' . esc_attr( $key ) . '" id="' . esc_attr( $key ) . '" value="' . esc_attr( $default ) . '" />';
		}

		public function time_picker_24( $key, $field, $default, $class ){
			echo '<input type="text" class="amt-metabox-picker amt-metabox-timepicker-24" name="' . esc_attr( $key ) . '" id="' . esc_attr( $key ) . '" value="' . esc_attr( $default ) . '" />';
		}
