...
if( !class_exists( 'AMT_Widget_Fields' ) ){
	...
	class AMT_Widget_Fields {
		...
		public static function display( $fields, $instance, $object ){
			foreach ( $fields as $key => $field ) {
				...
				if ( method_exists( __CLASS__, $field['type'] ) ) {
					echo '<div class="amt-widget-field">';
		...
		public static function image( $id, $name, $value, $label, $options ){
			...
			<input class="amt_upload_image upload_button_'. esc_attr( $id ) .' button-primary" type="button" value="' . esc_attr__( 'Choose Image', 'amt-framework' ). '" />
			<div class="amt_remove_image_wrap" style="'. esc_attr( $disstyle ) .'"><a href="#" class="amt_remove_image button" >' . esc_html__( 'Remove Image', 'amt-framework' ). '</a></div>
			</div>
			';
		}
	}
